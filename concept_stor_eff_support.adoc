...
| ボリュームインライン重複排除 | ○（デフォルト） | 使用不可 | サポート対象外 
...
ONTAP Select 9.6以降にアップグレードしたら、コマンドでアップグレードが完了したことが表示されるまで待って `system node upgrade-revert show`から、既存のボリュームのStorage Efficiencyの値を確認します。
...
シナリオ1:: アップグレード前に、ボリュームのStorage Efficiencyポリシーが有効になっていない場合：
+
--
* のボリューム `space guarantee = volume`で、インラインデータコンパクション、アグリゲートインライン重複排除、およびアグリゲートバックグラウンド重複排除が有効になっていません。これらのオプションは、アップグレード後に有効にできます。
* のボリューム `space guarantee = none`でバックグラウンド圧縮が有効になっていません。このオプションは、アップグレード後に有効にできます。
...
+
--
* のボリュームは、 `space guarantee = volume`アップグレード後も変更されません。
* のボリューム `space guarantee = none`で、アグリゲートバックグラウンド重複排除がオンになっています。
* のボリュームの `storage policy inline-only`ポリシーがautoに設定されています。
* ユーザ定義のStorage Efficiencyポリシーが設定されたボリュームでは、ポリシーに変更はありません。ただし、のボリュームは例外 `space guarantee = none`です。このボリュームでは、アグリゲートバックグラウンド重複排除が有効になっています
